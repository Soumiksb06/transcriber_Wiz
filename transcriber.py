...
def transcribe_in_batches(file_path: str, max_size_mb: int = 9) -> dict:
    ...
    Files that are less than or equal to 9 MB and 9 minutes (540 seconds)
    in duration are processed using transcribe_audio() (the same logic as small files).
    Otherwise, the file is split into 9‑minute segments and each segment is
    ...
    try:
        ...
        # If file is <= 9 MB and duration is <= 9 minutes (540 seconds), process as a small file.
        if file_size_mb <= max_size_mb and total_duration <= 9 * 60:
        ...
        total_batches = (int(total_duration) + batch_duration - 1) // batch_duration
        log.log(f"Processing {total_batches} batches...")
        
        # Process each batch sequentially
        for batch_num, start in enumerate(range(0, int(total_duration), batch_duration), 1):
            ...
            # Prepare ffmpeg command to extract a segment of the audio file
            ...
            if os.path.exists(batch_output):
                try:
                    ...
                    log.log(f"Error processing batch {batch_num}: {str(e)}")
            else:
